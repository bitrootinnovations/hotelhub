    ...
    <div class="page-header">
        <div class="add-item d-flex">
            <div class="page-title">
                ...
                <h6>Manage your restaurant staff</h6>
            </div>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="client-employees-table" class="table">
...
$(document).ready(function () {
    if ($.fn.DataTable.isDataTable('#client-employees-table')) {
        $('#client-employees-table').DataTable().destroy();
    }

    var table = $('#client-employees-table').DataTable({
        ajax: {
            url: '{{ route('client.employees.allData') }}',
            type: 'GET'
        },
        columns: [
            { data: 'id' },
            ...
            { data: 'phone', defaultContent: '-' },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return `<span class="badge ${cls} toggle-status" style="cursor:pointer"
                                data-id="${row.id}" data-status="${data}">${label}</span>`;
                }
            },
            {
                data: null,
                orderable: false,
                render: function (data, type, row) {
                    return `<div class="edit-delete-action d-flex gap-2">
                        <a class="border rounded d-flex align-items-center p-2"
                            href="employees/${row.id}/edit" title="Edit">
                        ...
                        <a class="border rounded d-flex align-items-center p-2 text-danger delete-btn"
                            href="javascript:void(0);" data-id="${row.id}" title="Delete">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
    ...
    $(document).on('click', '.toggle-status', function () {
        ...
        $.ajax({
            url: '{{ route('client.employees.updateStatus') }}',
            type: 'POST',
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        $.ajax({
            url: 'employees/' + empId + '/delete',
            type: 'POST',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            success: function (res) {
                if (res.success) {
                    $('#client-employees-table').DataTable().row(row).remove().draw();
                }
            },
            error: function () { alert('Error deleting staff member.'); }
        });
    });
});
</script>
