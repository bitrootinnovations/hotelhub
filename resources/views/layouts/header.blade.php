			...
			<div class="main-header">
				...
				<ul class="nav user-menu">
					...
					<li class="nav-item nav-item-box">
						<a href="{{ route('clients.map') }}" title="Client Locations Map">
					...
					@endif
					@if($isClientUser)
					{{-- Client: avatar initial + name + logout button --}}
					<li class="nav-item d-flex align-items-center gap-2 ms-auto">
						<span class="d-flex align-items-center gap-2">
							<span class="avatar avatar-sm rounded-circle bg-primary text-white d-flex align-items-center justify-content-center fw-bold fs-14" style="width:34px;height:34px;flex-shrink:0;">
								{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
							</span>
							<span class="fw-semibold fs-14 text-dark d-none d-sm-inline" style="max-width:140px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">
								{{ Auth::user()->name }}
							</span>
						</span>
						<form method="POST" action="{{ route('logout') }}" id="logoutForm">
							@csrf
							<button type="submit" class="btn btn-outline-danger btn-sm d-flex align-items-center gap-1" title="Logout">
					...
					<li class="nav-item dropdown has-arrow main-drop profile-nav">
						...
						<div class="dropdown-menu menu-drop-user">
							<div class="profileset d-flex align-items-center">
								...
								<div>
									<h6 class="fw-medium">{{ Auth::user()->name ?? 'Admin' }}</h6>
									<p>{{ Auth::user()->role->role_name ?? 'Admin' }}</p>
								</div>
							</div>
							<hr class="my-2">
							<form method="POST" action="{{ route('logout') }}" id="logoutForm">
								@csrf
								<a class="dropdown-item logout pb-0" href="#" onclick="document.getElementById('logoutForm').submit();">
