    ...
    <div class="page-header">
        <div class="add-item d-flex">
            <div class="page-title">
                <h4>Table Master</h4>
                <h6>Manage restaurant tables (T1, T2, A, B, etc.) per client</h6>
            </div>
        </div>
        ...
        <div class="page-btn">
            <a href="{{ route('tables.create') }}" class="btn btn-primary">
                <i class="ti ti-circle-plus me-1"></i>Add Table
            </a>
        </div>
        @endif
    </div>
...
$(document).ready(function () {
    if ($.fn.DataTable.isDataTable('#tables-table')) $('#tables-table').DataTable().destroy();
    var table = $('#tables-table').DataTable({
        ajax: { url: '{{ route('tables.allData') }}', type: 'GET' },
        columns: [
            { data: 'table_id' },
            { data: 'table_name' },
            ...
            { data: 'capacity' },
            {
                ...
                render: function (data, type, row) {
                    let cls = data == 1 ? 'bg-success' : 'bg-danger', lbl = data == 1 ? 'Available' : 'Inactive';
                    return `<span class="badge ${cls} toggle-status" style="cursor:pointer" data-id="${row.table_id}" data-status="${data}">${lbl}</span>`;
                }
            },
    ...
    $(document).on('click', '.toggle-status', function () {
        ...
        $.ajax({ url: '{{ route('tables.updateStatus') }}', type: 'POST',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
            data: { id, status: ns },
            success: function () { el.removeClass('bg-success bg-danger').addClass(ns ? 'bg-success' : 'bg-danger').text(ns ? 'Available' : 'Inactive').data('status', ns); }
        });
    });
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        $.ajax({ url: 'seating/' + id + '/remove', type: 'POST',
            headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
...
document.getElementById('custom-excel-export').addEventListener('click', function () {
    XLSX.writeFile(XLSX.utils.table_to_book(document.getElementById('tables-table'), { sheet: 'Tables' }), 'Tables.xlsx');
});
</script>
@endsection
